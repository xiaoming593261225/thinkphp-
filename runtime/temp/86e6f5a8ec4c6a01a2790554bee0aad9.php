<?php /*a:1:{s:30:"../Template/run/news\list.html";i:1553185739;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<div class="search-block" style="float: right;padding-top: 40px;">
    <form class="layui-form" action="<?php echo url('News/index'); ?>" method="post">
...
<table class="layui-table">
    ...
    <tr>
        ...
        <td><?php echo htmlentities($data['author']); ?></td>
        <td><?php echo htmlentities($data['news_name']); ?></td>
        <td><?php echo htmlentities($data['description']); ?></td>
        <td><img src="<?php echo htmlentities($data['icon']); ?>" style="width: 50px;"/></td>
        ...
        <td>
            <a href="javascript:;" class="layui-btn newsEdit layui-btn-radius" data-icon="<?php echo htmlentities($data['icon']); ?>"
               data-news_class_id="<?php echo htmlentities($data['news_class_id']); ?>" data-author="<?php echo htmlentities($data['author']); ?>"
               data-description="<?php echo htmlentities($data['description']); ?>" data-content="<?php echo htmlentities($data['content']); ?>" data-title="<?php echo htmlentities($data['title']); ?>"
               news_id="<?php echo htmlentities($data['id']); ?>"><i
...
<div id="doc" style="display: none;">
    <form class="layui-form" method="post" enctype="multipart/form-data" style="margin: 5%">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <select name="news_class_id" id="news_class_id" lay-verify="required">
                    ...
                    <option value="<?php echo htmlentities($fen['id']); ?>"><?php echo htmlentities($fen['news_name']); ?></option>
                    <?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
        ...
        <div class="layui-form-item layui-form-text">
            ...
            <script type="text/javascript" src="/static/run/static/ueditor/1.4.3/ueditor.config.js"></script>
            <!-- 编辑器源码文件 -->
            <script type="text/javascript" src="/static/run/static/ueditor/1.4.3/ueditor.all.js"></script>
            <!-- 实例化编辑器 -->
...
<script>
    ...
    $('.newsEdit').click(function () {
        ...
        $("#demo1").attr("src", icon);
        $.post('<?php echo url("newsShow"); ?>', {}, function (e) {
            console.log(e);
            var list = e;
            var len = e.length;
            var str = '';
    ...
    function newsDelete(t) {
        ...
        layer.confirm('确定要删除！', {btn: ['确认', '取消']}, function () {
            $.post('<?php echo url("newsDelete"); ?>', {id: id}, function (e) {
                if (e == 200) {
                    layer.msg('操作成功');
                    location.reload();
                } else {
